#!/usr/bin/env python3
"""Lineage-TF contribution-to-TOO vs background, on the rule-conformant cohort, ALL tumor fraction (no TF
filter). NOW RUNS BOTH COHORTS v1 AND v2 separately (user directive 2026-07-19), writing a `cohort` column
so the ggpubr renderer can emit one figure per cohort. All-TF companion to tf_contribution_dumbbell_rc_tf03.py
(the ONLY difference is that samples are not restricted by tumor fraction).
Out: results/rule_conformant/_ggpubr_{S,P}_rc_all.csv  (both cohorts, `cohort` column)"""
...
TFCUT = None; MINPOS = 10; COHORTS = ("v1", "v2")                          # None => all tumor fraction
...
def main():
    ...
    P.to_csv(f"{RC}/_ggpubr_P_rc_all.csv",index=False); S.to_csv(f"{RC}/_ggpubr_S_rc_all.csv",index=False)
    print(S[["cohort","channel","short","n_pos","master_meanz","p","stars"]].round(3).to_string(index=False),flush=True)
    print("TF_CONTRIB_DUMBBELL_RC_ALL_DONE",flush=True)
